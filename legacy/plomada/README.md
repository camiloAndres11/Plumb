# Plomada (legado, en producción)

> **Producto anterior, aislado en `legacy/plomada/`.** Sigue en producción
> (`plumb-duy6.onrender.com`, sitio estático en Render) y por eso se conserva
> entero y **no se refactoriza**: solo recibe arreglos de seguridad. Todo lo de
> aquí se corre **desde este directorio** (`cd legacy/plomada`), salvo los tests,
> que también corren desde la raíz con `python -m pytest` (ver `pyproject.toml`).
> Su CI es el job `legacy` de `.github/workflows/ci.yml`. Los datos crudos y el
> warehouse van en `legacy/plomada/data/` (no versionado).
>
> El README de la raíz describe el producto actual: Pliego.

<img src="plomada/static/logo.png" alt="Plomada" width="120" />
